<?php
	...
	}else{$query_all_sanpham = mysqli_query($conn, "SELECT * FROM `san_pham` ORDER BY `san_pham`.`id_sp` DESC LIMIT " . $sosp1trang . " OFFSET " . $offset);$tongsanpham = mysqli_query($conn, "SELECT * FROM san_pham");}
...
<div class="page_tat-ca-trang-suc">
                        ...
                        <div class="page"></div>
                            <ul class="pagination">
                              ...
                              <?php if ($page > 1) {
                                        $prev_page = $page -1 ?>
                              <?php } ?>
                              <li class="page-item"><a <?php if(isset($_GET['page']) && $_GET['page'] > 1 ) { ?> style="cursor: pointer;" <?php } ?> style="cursor:no-drop;" class="page-link" href="?xem=ds_tatcatrangsuc&menu_id=<?php echo $ht_anh_dm['menu_id'] ?>&page=<?= $prev_page ?>&<?=$param?>"><</a></li>
                              <!-- số page -->
                              ...
                              <?php if ($page < $tongPage) {
                                        $next_page = $page + 1;?>
                              <?php }?>
                              <li class="page-item"><a <?php if(isset($_GET['page']) && $_GET['page'] == $end_page ) { ?> style="cursor: no-drop;" <?php } ?> class="page-link" href="?xem=ds_tatcatrangsuc&menu_id=<?php echo $ht_anh_dm['menu_id'] ?>&page=<?= $next_page ?>&<?=$param?>">></a></li>
                            </ul>
                        </div>
                    </div>
                </div>
